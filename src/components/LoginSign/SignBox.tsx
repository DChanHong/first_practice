...
const SignBox = () => {
    ...
    const onSubmit:SubmitHandler<signFormData> = async (data) =>{
        ...
        await axios.post(`/customer` ,signData )
            .then((response)=> {
    ...
    return ( 
        <div>
            ...
            <div className='flex flex-wrap '> 
                        ...
                        <div className='flex-col '>
                            ...
                            <div>
                                <input {...register("email",
                                    ...
                                    className='border-2 w-80 py-3 rounded-2xl px-3 text-lg'  type="text" 
                                />
                                {errors?.email && <div className='ml-2 text-rose-500 text-sm'>{errors?.email?.message}</div>} 
                            </div>
                        </div>
                        <div className='flex-col '>
                            ...
                            <div>
                                ...
                                className='border-2 w-80 py-3 rounded-2xl px-3 text-lg'  type="password"  />
                                {errors?.password && <div className='ml-2 text-rose-500 text-sm'>{errors?.password?.message}</div>}
                            </div>
                        </div>
                        <div className='flex-col '>
                            ...
                            <div>
                                ...
                                className='border-2 w-80 py-3 rounded-2xl px-3 text-lg' type="password" />
                                {errors?.confirmPassword && <div className='ml-2 text-rose-500 text-sm'>{errors?.confirmPassword?.message}</div>}
                            </div>
                        </div>
                        <div className='flex-col '>
                            <div className='ml-2 mb-1 mt-2 font-bold '>이름</div>
                            <div>
                                <input {...register("name",{required:'이름을 작성하세요'})} 
                            className='border-2 w-80 py-3 rounded-2xl px-3 text-lg' type="text" />
                            </div>
                            {errors?.name &&<div className='ml-2 text-rose-500 text-sm'>{errors?.name?.message} </div> }
                        </div>
                        <div className='flex-col '>
                            <div className='ml-2 mb-1 mt-2 font-bold '>성별</div>
                            <select {...register("fm",{required:true})}
                ...
                <img className='w-[44rem] h-[44rem]' src='https://img.lovepik.com/photo/40024/1962.jpg_wh860.jpg'/>
                <img className='w-[44rem] h-[44rem]' src='https://img.lovepik.com/background/20211022/medium/lovepik-blue-technology-bitcoin-investment-banner-poster-background-image_605750671.jpg'/>
                </div>
            </div>
        </div>
        )
